...
function ParametresView({driversAmb,setDriversAmb,driversTpmr,setDriversTpmr,stagiairesAmb,setStagiairesAmb,formationTpmr,setFormationTpmr,vehicles,setVehicles,conventions,setConventions,equipements,setEquipements,transportTypes,setTransportTypes,preventifMissionTypes,setPreventifMissionTypes,bases,setBases,contacts,setContacts,plans,setPlans,tarifs,setTarifs,checklistsData,setChecklistsData,checklistEmails,setChecklistEmails,o2Emails,setO2Emails,peremptionEmails,setPeremptionEmails,preventifEmails,setPreventifEmails,listeRouge,setListeRouge,onBack,themeMode,toggleTheme}){
  ...
  const [newPreventifEmail,setNewPreventifEmail]=useState("");
        ...
        <div style={{flex:1,overflowY:"auto",padding:"24px"}}>
          ...
          {tab==="emails"&&(
            <div>
              ...
              <div style={{marginTop:28,paddingTop:20,borderTop:`1px solid ${C.border}`}}>
                <SectionTitle icon="📅" title="Emails — Préventif"/>
                <div style={{fontSize:11,color:C.muted,marginBottom:16}}>Ces adresses reçoivent la fiche complète d'un événement dès qu'elle est envoyée depuis l'écran terrain. Liste séparée des autres.</div>
                <div style={{display:"flex",gap:8,marginBottom:16}}>
                  <input value={newPreventifEmail} onChange={e=>setNewPreventifEmail(e.target.value)} placeholder="exemple@aps.be" style={{flex:1,background:C.bg,color:C.text,fontSize:13,border:`1.5px solid ${C.border}`,borderRadius:9,padding:"10px 13px",outline:"none"}}/>
                  <button onClick={()=>{if(newPreventifEmail.trim()&&newPreventifEmail.includes("@")){setPreventifEmails(p=>[...p,newPreventifEmail.trim()]);setNewPreventifEmail("");}}} style={{background:C.accentSoft,border:`1px solid ${C.accent}`,borderRadius:9,color:C.accent,padding:"10px 16px",fontSize:13,fontWeight:700,cursor:"pointer"}}>+ Ajouter</button>
                </div>
                {preventifEmails.length===0&&<div style={{fontSize:12,color:C.muted,textAlign:"center",padding:"20px 0"}}>Aucun destinataire configuré</div>}
                {preventifEmails.map((em,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:C.panel,border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 14px",marginBottom:7}}>
                    <span style={{fontSize:13}}>📅 {em}</span>
                    <button onClick={()=>setPreventifEmails(p=>p.filter((_,j)=>j!==i))} style={{background:C.dangerSoft,border:`1px solid ${C.danger}`,borderRadius:6,color:C.danger,padding:"5px 9px",fontSize:11,cursor:"pointer"}}>🗑</button>
                  </div>
                ))}
              </div>
            </div>
...
function calcTotalHeures(p){
  ...
  const d=parse(p.surPlace?p.heureDebutPrestation:p.heureDepartBase), f=parse(p.heureRetourBase);
...
function PreventifFicheDetail({ fiche, materiel, personnel, vehicles, transportTypes, preventifMissionTypes, radioCanaux, fiches, preventifEmails, bureau, readOnly, onSave, onDelete, onBack, themeMode, toggleTheme }){
  ...
  const checkCarnetBord=async()=>{
    ...
    for(const v of nonPrevVehicules){
      ...
      }catch(e){ results[v.vehicleId]=false; }
    }
    setCarnetBordOk(results);
  };
  useEffect(()=>{ checkCarnetBord(); }, [f.vehiculesEngages.length, f.date]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",color:C.text,display:"flex",flexDirection:"column"}}>
          ...
          {false?(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:6}}>
                  {[[true,"Oui"],[false,"Non"]].map(([v,l])=>(
                    <button key={l} onClick={()=>save({...f,subsistance:v})} style={{flex:1,padding:"7px",borderRadius:7,border:`1.5px solid ${f.subsistance===v?C.purple:C.border}`,background:f.subsistance===v?C.purpleSoft:"transparent",color:f.subsistance===v?C.purple:C.muted,fontWeight:700,fontSize:11,cursor:"pointer"}}>{l}</button>
                  ))}
                </div>
              </div>
            </div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:640}}>
            ...
            <tbody>
              {[...f.personnel].sort((a,b)=>{
                ...
                return(
                <tr key={p.id} style={isResponsable?{background:C.panel2}:p.surPlace?{background:"#ec489940"}:undefined}>
                  ...
                  {PREVENTIF_HEURE_FIELDS.map(hf=>(
                    <td key={hf} style={{padding:"4px 6px"}}>{
                      (hf==="heureDepartBase"&&p.surPlace)?<span style={{fontSize:11,color:C.muted}}>/</span>
                      :readOnly?<span style={{fontSize:11,color:C.text}}>{p[hf]||"—"}</span>
                      :<div style={{width:70}}><HeureInput value={p[hf]||""} onChange={v=>updatePersonnel(p.id,hf,v)}/></div>
                    }</td>
                  ))}
      ...
      {showDeleteConfirm&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
          <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:16,padding:24,width:360,maxWidth:"92vw"}}>
            <div style={{fontWeight:800,fontSize:16,marginBottom:8,color:C.danger}}>🗑 Supprimer la fiche ?</div>
            <div style={{fontSize:13,color:C.muted,marginBottom:20}}>"{f.nomEvenement}" sera définitivement supprimée. Cette action est irréversible.</div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowDeleteConfirm(false)} style={{flex:1,background:"transparent",border:`1px solid ${C.border}`,borderRadius:9,color:C.muted,padding:"11px",fontSize:13,cursor:"pointer"}}>Annuler</button>
              <button onClick={()=>{onDelete(f.id);onBack();}} style={{flex:1,background:C.danger,border:"none",borderRadius:9,color:"white",padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer"}}>Supprimer</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",color:C.text,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,padding:20,maxWidth:640,margin:"0 auto",width:"100%"}}>
        ...
        {todayFiches.length===0?(
          ...
          <button key={f.id} onClick={()=>setOpenFiche(f)} style={{width:"100%",background:C.panel,border:`1.5px solid ${C.purple}`,borderRadius:12,padding:"16px",marginBottom:10,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div>
            ...
            {bureau&&<div style={{width:12,height:12,borderRadius:"50%",flexShrink:0,background:f.statutBureau==="vert"?C.success:"#f59e0b"}}/>}
          </button>
        ))}
      </div>
    </div>
...
export default function App(){
  ...
  const [preventifEmails, setPreventifEmails] = useFirestoreState("preventifEmails", []);
